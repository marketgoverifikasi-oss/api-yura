...
export default async function angelnick(req: Request, res: Response) {
  ...
  try {
    const bg = await loadImage("https://raw.githubusercontent.com/kayzzaoshi-code/Uploader/main/file_1771170750646.jpeg")
    ...
    ctx.drawImage(userImg, 0, 0, userImg.width, userImg.height, centerX - circleSize, centerY - circleSize, circleSize * 2, circleSize * 2)
    ctx.restore()
    ...
    res.status(500).json({
      status: false,
      message: err.message || "Gagal membuat Angel Nick"
    })
  }
}
