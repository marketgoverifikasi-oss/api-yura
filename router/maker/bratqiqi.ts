...
function buildLines(ctx: any, tokens: any[], fontSize: number, maxW: number) {
  ...
  for (const token of tokens) {
    ...
    if (line.length > 0 && lineW + w > maxW) {
      while (line.length && line[line.length - 1].type === "space") { lineW -= line[line.length - 1].w; line.pop() }
      lines.push({ items: line, width: lineW })
      line = [{ ...token, w }]
      lineW = w
    } else { line.push({ ...token, w }); lineW += w }
  }
...
export default async function bratqiqi(req: Request, res: Response) {
  try {
    const { text } = req.query
    if (!text || typeof text !== "string") return res.status(400).json({ status: false, message: "text required" })

    const bgUrl = "https://raw.githubusercontent.com/kayzzaoshi-code/Uploader/main/file_1771532869746.jpeg"
    ...
    const centerY = canvas.height / 2 + 108
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (const token of line.items) {
        if (token.type === "text") { ctx.fillText(token.value, currentX, y); currentX += token.w }
        else if (token.type === "space") currentX += token.w
